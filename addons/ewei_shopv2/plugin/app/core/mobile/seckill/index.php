...
class Index_EweiShopV2Page extends AppMobilePage
{
    public function get_list()
    {
        ...
        foreach( $alltimes as $key => $time ) 
        {
            ...
            if( $endtime < $currenttime && !$room["oldshow"] ) 
            {
                $oldshow = false;
            }
        ...
        $seckill_adv = array(  );
        $page = $this->model->getPage("seckill", true);
        if( !empty($page) ) 
        {
            ...
            $seckill_style = $page["data"]["page"]["seckill"]["style"];
            $seckill_color = $page["data"]["page"]["seckill"]["color"];
        }

        app_json(array( "diypages" => $diypages, "timeindex" => $timeindex, "times" => $times, "rooms" => $rooms, "roomid" => $roomid, "taskid" => $taskid, "timeid" => $timeid, "seckill_style" => $seckill_style, "seckill_color" => $seckill_color ));
    }
    ...
    public function get_goods()
    {
        ...
        app_json(array( "diypage" => $diypage, "time" => $time, "goods" => $goods ));
    }
    ...
    public function getTodaySeckill()
    {
        ...
        $year = date("Y");
        $month = date("m");
        return redis()->hGet((string) $redis_prefix . "calendar_" . $year . "_" . $month, date("Y-m-d"));
    }
    ...
    public function getTaskInfo($taskid)
    {
        ...
        $redis = redis();
        $redis_prefix = $this->get_prefix();
        $info = $redis->hGetAll((string) $redis_prefix . "info_" . $taskid);
        if( empty($info) ) 
        {
            return false;
        }
    ...
    public function getRooms($taskid)
    {
        ...
        $redis_prefix = $this->get_prefix();
        $allrooms = array(  );
        $rooms = $redis->lGetRange((string) $redis_prefix . "rooms_" . $taskid, 0, -1);
    ...
    public function getTaskTimes($taskid)
    {
        ...
        $redis_prefix = $this->get_prefix();
        $times = $redis->lGetRange((string) $redis_prefix . "times_" . $taskid, 0, -1);
        $alltimes = array(  );
        if( empty($times) ) 
        {
            return false;
    ...
    public function getSeckillGoods($taskid, $time, $goodsid)
    {
        ...
        $redis_prefix = $this->get_prefix();
        $timegoods = array(  );
        $goods = redis()->hGetAll((string) $redis_prefix . "goods_" . $taskid);
        if( empty($goods) ) 
        {
            return false;
        }
    ...
    public function getSeckill($goodsid, $optionid = 0, $realprice = true, $openid = "")
    {
        ...
        if( !empty($timegoods) ) 
        {
            ...
            {
                if( $currenttime < $goods_starttime ) 
                {
                    $status = 1;
                }
                else
    ...
    public function deleteSeckill()
    {
        ...
        $redis = redis();
        $redis_prefix = $this->get_prefix();
        $keys = $redis->keys((string) $redis_prefix . "queue_*");
        $orders = array(  );
        foreach( $keys as $key ) 
        {
    ...
    public function getSeckillCount($taskid, $timeid, $goodsid, $optionid = 0, $openid = "")
    {
        ...
        $date = date("Y-m-d");
        $redis_prefix = $this->get_prefix();
        $keys = $redis->keys((string) $redis_prefix . "queue_" . $date . "_" . $taskid . "_" . $timeid . "_" . $goodsid . "_*");
        if( empty($keys) ) 
        {
