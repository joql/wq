...
class Goods_EweiShopV2Page extends AppMobileAuthPage
{
	...
	public function get_list()
	{
		...
		if (0 < $total) {
			...
			$sql = 'SELECT g.id, g.title, g.thumb, g.merchid, g.minprice, g.maxprice, g.total, g.sales FROM ' . tablename('ewei_shop_goods') . 'g' . $condition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . $presize . ',' . $psize;
