...
class Merch_EweiShopV2Page extends AppMobilePage
{
	public function get_list()
	{
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			...
			$city = $_GPC['city'];
			if(empty($city) && $citysel && $pindex==1 && $_GPC['firsttime'] && !empty($lat) && !empty($lng)){
				$pos = m('util')->location($lat, $lng);
				if(!empty($pos) && !empty($pos['result']) && !empty($pos['result']['addressComponent']['city'])){
					$city = $pos['result']['addressComponent']['city'];
				}
			}
			...
			if (!empty($_GPC['province']) && $citysel) 
			{
				$data['province'] = $_GPC['province'];
			}
			if (!empty($city) && $citysel) 
			{
				$data['city'] = $city;
			}
		...
		app_json(array('list' => $merchuser, 'total' => intval($total), 'pagesize' => intval($psize), 'cates'=>$category, 'disopt'=>$disopt, 'citysel'=>$citysel, 'citys'=>$citys, 'selcity'=>$city));
	}
