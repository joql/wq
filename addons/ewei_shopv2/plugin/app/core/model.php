	...
	class AppModel extends PluginModel
	{
					...
					foreach ($page['data']['items'] as $itemid => &$item ) {
						...
						if ($mobile) {
							...
							if ($item['id'] == 'richtext') {
							 ...
							 else if (($item['id'] == 'topmenu') && is_array($item['params']) && !(empty($item['params']['content']))) {
								if (!(empty($item['data']))) {
									$swiperpage = ((empty($item['style']['pagenum']) ? 8 : $item['style']['pagenum']));
									$data_temp = array();
									$k = 0;
			...
			if ($item['id'] == 'goods') {
				if (empty($item['params']['goodsdata']) && !(empty($item['data'])) && is_array($item['data'])) {
					...
					if (!(empty($group)) && !(empty($group['goodsids']))) {
						...
						$item['params']['goodsnum'] = ((!(empty($item['params']['goodsnum'])) ? $item['params']['goodsnum'] : 20));
						$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice, video,`type` from ' . tablename('ewei_shop_goods') . ' where id in( ' . $group['goodsids'] . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $item['params']['goodsnum'], array(':uniacid' => $_W['uniacid']));
					}
						...
						foreach ($item['data'] as $childid => $childgoods ) {
							foreach ($goods as $index => $good ) {
								if ($good['id'] == $childgoods['gid']) {
									...
									$item['data'][$childid] = array('gid' => $good['id'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'thumb' => $good['thumb'], 'total' => $good['total'], 'productprice' => $good['productprice'], 'ctype' => $good['type'], 'sales' => $good['sales'] + intval($good['salesreal']), 'video' => $good['video'], 'bargain' => $good['bargain']);
								}
		...
		public function getAuth()
		{
			...
			//$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . ('xcxapi/auth-xcx-info/validate?site_id=' . $siteid . '&uniacid=' . $_W['uniacid']));
          	$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . ('index/index/getAuth?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid']));
		...
		global $_W;
			...
			load()->func('communication');
                	...
                	$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . 'index/index/getRelease.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid']);
		...
		public function getReleaseList()
		{
			...
			load()->func('communication');
                	...
                	$request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . 'index/index/getReleasList?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid']);
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '')
		{
			...
			if ($message_type == TM_CYCELBUY_SELLER_DATE) {
				...
				$message = array(
					...
					'keyword2' => array('title' => '业务状态', 'value' => '修改收货时间通知', 'color' => '#000000'),
					'keyword3' => array('title' => '业务内容', 'value' => $data['nickname'] . '修改了收货时间', 'color' => '#000000'),
				...
				$message = array(
					...
					'keyword2' => array('title' => '业务状态', 'value' => '修改收货地址通知', 'color' => '#000000'),
					'keyword3' => array('title' => '业务内容', 'value' => $data['nickname'] . '修改了收货地址', 'color' => '#000000'),
					'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
					'remark'   => array('value' => "\n" . '感谢您的支持', 'color' => '#000000')
					);
				$datas[] = array('name' => '昵称', 'value' => $data['nickname']);
				...
				$message = array(
					...
					'keyword2' => array('title' => '业务状态', 'value' => '修改收货时间通知', 'color' => '#000000'),
					'keyword3' => array('title' => '业务内容', 'value' => '您的收货时间已修改', 'color' => '#000000'),
					'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
					'remark'   => array('value' => "\n" . '感谢您的支持', 'color' => '#000000')
					);
				$toopenid = $openid;
				...
				$message = array(
					...
					'keyword2' => array('title' => '业务状态', 'value' => '修改收货地址通知', 'color' => '#000000'),
					'keyword3' => array('title' => '业务内容', 'value' => '您的收货地址修改已完成', 'color' => '#000000'),
					'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
					'remark'   => array('value' => "\n" . '感谢您的支持', 'color' => '#000000')
					);
				$toopenid = $openid;
