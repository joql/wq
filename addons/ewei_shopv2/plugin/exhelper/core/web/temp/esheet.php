...
class Esheet_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$id = intval($_GPC['id']);
			$data = array('esheetid' => trim($_GPC['esheetid']), 'esheetname' => trim($_GPC['esheetname']), 'customername' => trim($_GPC['customername']), 'customerpwd' => trim($_GPC['customerpwd']), 'monthcode' => trim($_GPC['monthcode']), 'sendsite' => trim($_GPC['sendsite']), 'paytype' => intval($_GPC['paytype']), 'templatesize' => trim($_GPC['templatesize']), 'isnotice' => intval($_GPC['isnotice']), 'issend' => intval($_GPC['issend']), 'isdefault' => intval($_GPC['isdefault']));
