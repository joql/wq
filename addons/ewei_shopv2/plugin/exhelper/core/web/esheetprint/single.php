...
class Single_EweiShopV2Page extends PluginWebPage
{
	...
	public function getdata()
	{
		...
		if ($_W['ispost']) {
			$condition = ' o.uniacid = :uniacid and m.uniacid = :uniacid and o.deleted=0 and o.addressid<>0 and o.merchid=0 and o.isparent=0 and o.storeid=0 and o.virtual=0';
	...
	public function getesheetTemp()
	{
		...
		if ($_W['ispost']) {
			...
			if ($eorder['OrderCode'] == '0') {
				$ordercode = m('common')->createNO('order', 'ordersn', 'ES');
			...
			$commodityOne = array();
			$commodityOne['GoodsName'] = trim($_GPC['printarr']['sendinfo']);
			$commodity = array();
			...
			if ($result['ResultCode'] == '100') {
				...
				$data = array('express' => trim($esheet_item['express']), 'expresssn' => trim($result['Order']['LogisticCode']), 'expresscom' => trim($esheet_item['name']), 'sendtime' => time(), 'refundstate' => 0);
				...
				pdo_update('ewei_shop_order', $data, array('id' => explode(',', $_GPC['printarr']['orderid'])));
				exit(json_encode(array('result' => 'success', 'PrintTemplate' => $result['PrintTemplate'], 'ordercode' => $eorder['OrderCode'], 'issend' => $esheet_item['issend'])));
			}
			 else {
