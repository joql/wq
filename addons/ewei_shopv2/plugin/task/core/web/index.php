...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	public function main_new()
	{
		...
		$taskS[2] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_task_record') . ' where uniacid = ' . $_W['uniacid'] . ' and finishtime = \'0000-00-00 00:00:00\' and stoptime < \'' . $date . '\'');
	...
	public function post_new()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($diy['verb']) || empty($diy['unit'])) {
				show_json(0, '前缀和后缀不能为空');
			}


			pdo_update('ewei_shop_task_type', array('verb' => $diy['verb'], 'unit' => $diy['unit']), array('type_key' => 'poster'));
			...
			$poster_text = pdo_get('ewei_shop_task_type', array('type_key' => 'poster'), array('verb', 'unit'));
