...
class Common_EweiShopV2Model
{
	...
	public function wechat_build($params, $wechat, $type = 0)
	{
		...
		if ($payment['is_new'] == 1) {
			...
			if ($payment['type'] == 4) {
				$params = array('service' => 'pay.weixin.jspay', 'body' => trim($params['title']), 'out_trade_no' => $params['tid'], 'total_fee' => $params['fee'], 'openid' => (empty($params['openid']) ? trim($_W['openid']) : trim($params['openid'])));
