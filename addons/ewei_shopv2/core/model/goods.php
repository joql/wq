...
class Goods_EweiShopV2Model
{
	...
	public function getList($args = array())
	{
		...
		if (!($random)) {
			$sql = 'SELECT id,title,subtitle,thumb,thumb_url' . $officsql . ',marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay' . "\r\n" . '            FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
		...
		foreach ($list as $key => $value ) {
			...
			if ($value['nocommission'] == 0) {
				...
				$list[$key]['seecommission'] = $this->getCommission($value, $level, $set);
				$list[$key]['cansee'] = $set['cansee'];
				$list[$key]['seetitle'] = $set['seetitle'];
			}
			 else {
				$list[$key]['seecommission'] = 0;
				$list[$key]['cansee'] = $set['cansee'];
				$list[$key]['seetitle'] = $set['seetitle'];
			}
		}
	...
	public function visit($goods = array(), $member = array())
	{
		...
		if ((strpos($member['groupid'], ',') !== false) || (strpos($showgroups, ',') !== false)) {
		 ...
		 else if (strpos($showgroups, $member['groupid']) !== false) {
			$isShow = true;
		}
