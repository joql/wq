...
}if (!(function_exists('get_wxpay_sign'))) {	/**     * 生成微信支付签名     * @param array $data    签名数据     * @param string $key    支付密钥     */	function get_wxpay_sign($data, $key)	{		$data = array_filter($data);		ksort($data);		$string_a = http_build_query($data);		$string_a = urldecode($string_a);		$string_sign_temp = $string_a . '&key=' . $key;		$sign = md5($string_sign_temp);		$result = strtoupper($sign);		return $result;	}}
...
function cloud_upgrade($type, $post_data = array(), $timeout = 60)
{
    ...
    $resp = ihttp_request("http://vip.zhifun.cc/api/api.php", $post_data, $extra, $ip, $timeout);
