...
class Release1_EweiShopV2Page extends SystemPage
{
	...
	public function audit()
	{
		...
		if ($action == 'upload') {
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . 'code-manage/submit-only?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&ismanage=1');
		}
		else {
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . 'code-manage/audit-only?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&ismanage=1', array());
		}
	...
	public function auth()
	{
		...
		header('Location:' . EWEI_SHOPV2_AUTH_WXAPP . 'auth/auth?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&ismanage=1');
	}
